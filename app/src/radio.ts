...
class Volume {
     ...
     constructor() {
          ...
          this.slider.on('input', () => {
               ...
               if (radioPlayer.volume != 0) {
                    this.speakerLogo.attr(
                         'src',
                         '/assets/sprites/icons/volume.svg'
                    )
               } else {
                    this.speakerLogo.attr(
                         'src',
                         '/assets/sprites/icons/mute.svg'
                    )
               }
          })
     ...
     toggleVolume() {
          if (radioPlayer.volume != 0) {
               ...
               state.volumeValue = 0
               this.slider.val(0)
               this.speakerLogo.attr('src', '/assets/sprites/icons/mute.svg')
          } else {
               // unmute
               ...
               this.speakerLogo.attr('src', '/assets/sprites/icons/volume.svg')
          }
     }
}
...
export const radioInit = () => {
     try {
          ...
          if (
               radioPlayer.canPlayType('audio/aac') != 'no' &&
...
const radioPlay = (channel?: string) => {
     ...
     if (playerReady) {
          ...
          radioPlayer.onloadstart = function () {
               if (nowPlaying) setTempTitle('Поиск частоты...')
          }
...
const getTrackHistory = () => {
     ...
     const amount = Math.floor(
          calculateHistoryViewport() / trackHistoryItemHeight
     )
     ...
     switch (+data['status']) {
          case 0:
               data['payload'].forEach(function (track) {
                    ...
                    // Time
                    historyHtml +=
                         '<div class="air-playlist-item"><div class="air-time">' +
                         (trackDate.getHours() < 10 ? '0' : '') +
                         trackDate.getHours() +
                         ':' +
                         (trackDate.getMinutes() < 10 ? '0' : '') +
                    ...
                    if (
                         track['artist_links'] &&
                         track['artist_links'].length > 0
                    ) {
                         const artistLink =
                              track['artist_links'][0]['link_text']
                         historyHtml +=
                              '<a class="air-band" href="' +
                              artistLink +
                              '" target="_blank">' +
                              track['artist'] +
                              '</a>'
          ...
          default:
               historyHtml =
                    '<div class="history-error">Не удалось получить историю эфира, ошибка #' +
                    data['status'] +
                    ' (' +
                    data['payload'] +
                    ')</div>'
     ...
     if (typeof track === 'string') {
          ...
          switch (track['status']) {
               ...
               case 1: // server couldn't parse track info and sends us what it had
                    ...
                    trackToDisplay =
                         showingTrackStruct['payload']['artist'] +
                         ' – ' +
                         showingTrackStruct['payload']['title']
...
export async function changeStream(direction: number) {
     ...
     if (
          carouselIndex <= channels.length - 1 &&
